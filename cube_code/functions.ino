...
void angle_calc() {
  ...
  if (abs(angleX - offsets.X1) < 2 && abs(angleY - offsets.Y1) < 2) {
    balancing_point = 1;
    if (!vertical) beep();
    vertical = true;
    ...
    if (!vertical) beep();
    vertical = true;
  } else if (abs(angleX - offsets.X3) < 3 && abs(angleY - offsets.Y3) < 0.6) {
    balancing_point = 3;
    if (!vertical) beep();
    vertical = true;
...
void XY_to_threeWay(float pwm_X, float pwm_Y) {
  ...
  int16_t m1 = round(0.5 * pwm_X - 0.6 * pwm_Y); 
  int16_t m2 = round(0.5 * pwm_X + 0.6 * pwm_Y);
  int16_t m3 = -pwm_X;  
    ...
    case 'c':
      ...
      if (cmd == '-' && calibrating)  {
        ...
        if (abs(robot_angleX) < 10 && abs(robot_angleY) < 10) {
          ...
          save();
        } else if (abs(robot_angleX) < 15 && robot_angleY > -48 && robot_angleY < -28) {
          offsets.ID4 = 99;
